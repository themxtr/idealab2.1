 ...
 * Receives the authorization code from Google and redirects to frontend
 */
export default function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    return res.redirect(
      `${getOrigin(req)}/#/auth/google/callback?code=${encodeURIComponent(code as string)}${state ? `&state=${encodeURIComponent(state as string)}` : ''}`
    );
  } catch (error) {
